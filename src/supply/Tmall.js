...
async function getItemName(page) {
    let title = await page.$eval('.tb-detail-hd > h1', e => e.innerText);
...
async function getGoodSwiper(page) {
    const goodSwiper = await page.$$eval('#J_UlThumb li', e => {
        ...
        for (let i = 0; i < e.length; i++) {
            src = e[i].children[0].children[0].src.replace('_60x60', '_600x600');
            arr.push(src);
        }
        return arr;
    });
...
async function getItemEffect(page) {
    let effect = await page.$$eval('#J_AttrUL li', e => {
        let a = '...';
        e.forEach(li => {
...
async function getItemBody(page) {
    item.body = await page.$eval('#description', e => e.innerHTML);
