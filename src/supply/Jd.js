...
async function startCollection(page) {
    ...
    auth.getToken().then(async (managerToken) => {
        token = managerToken;
        logger.info('开始分析页面数据');

        await getItemName(page);
        await page.waitForSelector('#spec-list');
...
async function getItemName(page) {
    let title = await page.$eval('.itemInfo-wrap .sku-name', e => e.innerText);
...
async function getGoodSwiper(page) {
    const goodSwiper = await page.$$eval('#spec-list li > img', e => {
        let arr = [];
        for (let i = 0; i < e.length; i++) {
            src = e[i].src.replace('/n5/', '/n0/').replace('s75x75_', '');
            arr.push(src);
        }
        return arr;
    });
...
async function getItemEffect(page) {
    let effect = await page.$$eval('.p-parameter li', e => {
        let a = '...';
        e.forEach(li => {
...
async function getItemBody(page) {
    item.body = await page.$eval('.detail-content-wrap', e => {
        console.info(e);
        return e.innerHTML;
    });
